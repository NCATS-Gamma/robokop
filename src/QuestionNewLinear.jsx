...
import QuestionNewPres from './components/questionNewLinear/QuestionNewPres';
import CardTypes from './components/util/questionNewCardTypes';

class QuestionNewLinear extends React.Component {
  constructor(props) {
    super(props);
  ...
  onCreate() {
    const oldMachineQuestion = this.state.machineQuestion;

    const nodes = [];
    for (let i = 0; i < oldMachineQuestion.length; i += 1) {
      const node = oldMachineQuestion[i];
    ...
    const edges = [];
    for (let i = 1; i < oldMachineQuestion.length; i += 1) {
      const node = oldMachineQuestion[i];
      const edge = {
        source_id: i - 1,
        target_id: i,
...
export default QuestionNewLinear;