...
class About extends React.Component {
  ...
  componentDidMount() {
    this.appConfig.user(
      data => this.setState({ user: data, ready: true }),
      (err) => {
  ...
  render() {
    const { ready, user, licenses } = this.state;
    return (
      <div>
        {ready ? (
          <div>
            ...
            <Grid style={{ marginBottom: '50px' }}>
              <Row>
                <Col md={8}>
                  ...
                  <p>
                    An extensive list of Robokop resources can be found at <a target="_blank" rel="noopener noreferrer" href="https://researchsoftwareinstitute.github.io/data-translator/apis">RSI Data Translator</a> or users can view all publicly available APIs by going to the <a href="/apidocs/">APIDocs</a> page.
                  </p>
                  <hr />
                  <h3>Open Source</h3>
                  <p>This project is open source. All the code can be found on GitHub: <a href="https://github.com/NCATS-Gamma">Robokop</a></p>
                      ...
                      <Panel.Body>
                        {licenses.map(license => (
                          <div key={license.name}>
                            <hr />
                            <h3>{license.name}</h3>
                            <p>Home: <a href={license.url}>{license.url}</a></p>
                            <p>Type: {license.license}</p>
                            <p>License: <a href={license.license_url}>{license.license_url}</a></p>
                            <p>Citation: <a href={license.citation_url}>{license.citation_url}</a></p>
                          </div>
                        ))}
                      </Panel.Body>
                    </Panel.Collapse>
